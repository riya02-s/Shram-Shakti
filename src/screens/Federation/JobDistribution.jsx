    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-gray-200">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900 mt-1">Job Distribution Analytics</h1>
          <p className="text-sm text-gray-600">
            Audit job assignment equity across cooperative workers to verify workload fairness scoring.
          </p>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-3">Fairness Score</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredData.map((worker) => (
                <tr key={worker.workerName} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-3.5 font-medium text-gray-900">{worker.workerName}</td>
                  <td className="px-6 py-3.5 text-gray-600">
                    <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                      {worker.category}
                    </span>
                  </td>
                  <td className="px-6 py-3.5 text-center font-semibold text-gray-800">{worker.jobsAssigned}</td>
                  <td className="px-6 py-3.5 text-center text-emerald-600 font-semibold">{worker.jobsCompleted}</td>
